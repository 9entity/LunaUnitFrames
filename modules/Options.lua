...
function LUF:CreateConfig()
	...
	local moduleOptions = {
		...
		["powerBar"] = {
			...
			args = {
				...
				fivesecond = {
					...
					type = "toggle",
					order = 4,
					hidden = function(info) return info[1] ~= "player" or select(2, UnitClass("player")) == "WARRIOR" or select(2, UnitClass("player")) == "ROGUE" or select(2, UnitClass("player")) == "DEATHKNIGHT" end
				},
				background = {
					name = BACKGROUND,
					desc = string.format(L["Enable or disable the %s."], BACKGROUND),
			...
			args = {
				...
				raidrole = {
					...
					args = {
						enabled = {
							name = ENABLE,
							desc = string.format(L["Enable or disable the %s."],L["Raidrole"]),
							type = "toggle",
							order = 1,
						},
						size = {
		...
		["ghoul"] = {
			...
			type = "group",
			order = -4,
			hidden = function(info) return info[1] ~= "player" or (select(2,UnitClass("player")) ~= "DEATHKNIGHT") end,
			--inline = true,
			args = {
				enabled = {
					name = ENABLE,
					desc = string.format(L["Enable or disable the %s."],L["Ghoul"]),
					type = "toggle",
					order = 1,
				},
				autoHide = {
		...
		["comboPoints"] = {
			...
			order = -2,
			--inline = true,
			hidden = function(info) return (info[1] ~= "target" and info[1] ~= "player") end,
			args = {
				enabled = {
					name = ENABLE,
					desc = string.format(L["Enable or disable the %s."],COMBO_POINTS),
				...
				showTarget = {
					name = TARGET.." "..COMBO_POINTS,
					desc = L["Show Combo Points you have on your target"],
					type = "toggle",
					order = 3,
					hidden = function(info) return info[1] ~= "player" end,
				},
	...
	local aceoptions = {
		...
		args = {
				...
				args = {
					...
					GeneralOptions = {
						...
						args = {
							...
							hidepetinvehicle = {
								name = L["Hide Pet in Vehicle"],
								desc = L["Hide the Pet Frame while riding a vehicle"],
								type = "toggle",
								order = 2.7,
								get = function(info) return LUF.db.profile.units.pet.hidepetinvehicle end,
								set = function(info, value) LUF.db.profile.units.pet.hidepetinvehicle = value LUF.stateMonitor:SetAttribute("hidepetinvehicle",value and true) end,
								disabled = Lockdown,
							},
							slots = {
								name = L["Bars"],
					...
					GeneralOptions = {
						...
						args = {
							...
							enabled10 = {
								...
								width = "half",
								disabled = Lockdown,
								set = function(info, value) LUF.db.profile.units.raid.filters[10] = value LUF:SetupHeader("raid10") LUF:UpdateMovers() end,
								get = function(info, value) return LUF.db.profile.units.raid.filters[10] end,
								hidden = function() return LUF.db.profile.units.raid.groupBy ~= "CLASS" end,
							},
							raid10 = {
								name = L["Anchor To"],
								...
								order = 2.52,
								values = getAnchors,
								get = function() return LUF.db.profile.units.raid.positions[10].anchorTo end,
								set = SetAnchorTo,
								disabled = Lockdown,
								hidden = function() return LUF.db.profile.units.raid.groupBy ~= "CLASS" end,
							},
							x10 = {
								...
								width = 0.8,
								validate = nbrValidate,
								get = function() return tostring(LUF.db.profile.units.raid.positions[10].x) end,
								set = function(info, value) LUF.db.profile.units.raid.positions[10].x = tonumber(value) LUF:PlaceFrame(LUF.frameIndex["raid10"]) end,
								disabled = Lockdown,
								hidden = function() return LUF.db.profile.units.raid.groupBy ~= "CLASS" end,
							},
							y10 = {
								...
								width = 0.8,
								validate = nbrValidate,
								get = function() return tostring(LUF.db.profile.units.raid.positions[10].y) end,
								set = function(info, value) LUF.db.profile.units.raid.positions[10].y = tonumber(value) LUF:PlaceFrame(LUF.frameIndex["raid10"]) end,
								disabled = Lockdown,
								hidden = function() return LUF.db.profile.units.raid.groupBy ~= "CLASS" end,
							},
							slots = {
			...
			boss = {
				...
				type = "group",
				order = 27,
				arg = LUF.db.profile.units.boss,
				childGroups = "tab",
				args = {
					enabled = {
						name = ENABLE,
						desc = string.format(L["Enable the %s frame(s)"], L["boss"]),
						type = "toggle",
						order = 1,
						disabled = Lockdown,
						set = setEnableUnit,
			...
			hidden = {
				...
				args = {
					...
					boss = {
						name = BOSS,
						desc = string.format(L["Hides the default %s frame"], BOSS),
						type = "toggle",
						order = 12,
					},
				},
